...
import { listIncomingWebhooks, getIncomingWebhook, createIncomingWebhook, updateIncomingWebhook, deleteIncomingWebhook } from '@lyeve/cms-client-rest';
...
describe('REST incoming webhooks', () => {
  it('listIncomingWebhooks GETs /api/admin/incoming-webhooks', async () => {
    const { client, fetchFn } = mkClient([{ id: 'iw1', name: 'Form Submit', schema_name: 'contact' }]);
  ...
  it('getIncomingWebhook encodes the webhook id', async () => {
    ...
    expect(fetchFn.mock.calls[0][0]).toBe('/api/admin/incoming-webhooks/iw%2F1');
  });

  it('createIncomingWebhook POSTs input to /api/admin/incoming-webhooks', async () => {
    const { client, fetchFn } = mkClient({ id: 'iw2', name: 'Slack Command', schema_name: 'slack' });
    const input = { name: 'Slack Command', schema_name: 'slack', secret: 's3cret', field_map: { text: 'message' }, enabled: true, allowed_ips: ['1.2.3.4'] };
  ...
  it('updateIncomingWebhook PUTs input to /api/admin/incoming-webhooks/{id}', async () => {
    const { client, fetchFn } = mkClient({ id: 'iw1', name: 'Updated', schema_name: 'contact' });
