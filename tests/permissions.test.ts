...
import { listPermissions, upsertPermission, deletePermission } from '@lyeve/cms-client-rest';
...
describe('REST permissions', () => {
  it('listPermissions GETs /api/admin/permissions', async () => {
    const { client, fetchFn } = mkClient([{ id: 'p1', role: 'editor', schema_name: 'articles', actions: ['read'], field_mask: [] }]);
  ...
  it('upsertPermission POSTs data to /api/admin/permissions', async () => {
    const { client, fetchFn } = mkClient({ id: 'p2', role: 'admin', schema_name: '*', actions: ['read', 'write'], field_mask: [] });
    const input = { role: 'admin', schema_name: '*', actions: ['read', 'write'], field_mask: [] };
