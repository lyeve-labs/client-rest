...
describe('REST plugins - status & schema', () => {
  it('getPluginStatus GETs /api/admin/plugins/status', async () => {
    const { client, fetchFn } = mkClient({ compiled: ['content', 'media'], entitled: ['content'], plugins: [] });
  ...
  it('getPluginSchema GETs /api/admin/plugins/{name}/schema', async () => {
    const { client, fetchFn } = mkClient({ type: 'object', properties: { api_key: { type: 'string' } } });
  ...
  it('getPluginSchema encodes the plugin name', async () => {
    ...
    expect(fetchFn.mock.calls[0][0]).toBe('/api/admin/plugins/my%20plugin/schema');
  });
});

describe('REST plugins - config', () => {
  it('getPluginConfig returns config for existing plugin', async () => {
    const { client, fetchFn } = mkClient({ api_key: 'sk-xxx', endpoint: 'https://api.example.com' });
    const config = await getPluginConfig('my-plugin', client);
    expect(config).toEqual({ api_key: 'sk-xxx', endpoint: 'https://api.example.com' });
    expect(fetchFn.mock.calls[0][0]).toBe('/api/admin/plugins/my-plugin/config');
    expect(fetchFn.mock.calls[0][1].method).toBe('GET');
  });
  ...
  it('getPluginConfig rethrows non-404 errors', async () => {
    ...
    await expect(getPluginConfig('my-plugin', client)).rejects.toThrow(ApiError);
  });
  ...
  it('resetPluginConfig POSTs to /api/admin/plugins/{name}/config/reset', async () => {
    ...
    expect(fetchFn.mock.calls[0][0]).toBe('/api/admin/plugins/my-plugin/config/reset');
...
describe('REST plugins - operations', () => {
  ...
  it('getMigrationCompat GETs /api/admin/plugins/{name}/migration-compat?target=V', async () => {
    ...
    expect(fetchFn.mock.calls[0][0]).toBe('/api/admin/plugins/my-plugin/migration-compat?target=v2.0.0');
    expect(fetchFn.mock.calls[0][1].method).toBe('GET');
  });
  ...
  it('getMigrationCompat encodes the target version', async () => {
    ...
    expect(fetchFn.mock.calls[0][0]).toBe('/api/admin/plugins/my-plugin/migration-compat?target=v2.0.0-beta');
  });

  it('safeUpgradePlugin POSTs to /api/admin/plugins/{name}/safe-upgrade', async () => {
    const { client, fetchFn } = mkClient({ compatible: true, new_migrations: ['001_init'] });
    const result = await safeUpgradePlugin('my-plugin', client);
    expect(result.compatible).toBe(true);
    expect(fetchFn.mock.calls[0][0]).toBe('/api/admin/plugins/my-plugin/safe-upgrade');
    expect(fetchFn.mock.calls[0][1].method).toBe('POST');
    expect(JSON.parse(fetchFn.mock.calls[0][1].body)).toEqual({});
  });
});
...
describe('REST plugins - changelog (raw fetch)', () => {
  it('getChangelog fetches /api/admin/plugins/{name}/changelog with raw fetch', async () => {
    const fetchFn = vi.fn(async (_url: string): Promise<Response> =>
      new Response('# Changelog\n\n## v1.0.0\n- Initial release', { status: 200, headers: { 'content-type': 'text/markdown' } }));
    const text = await getChangelog('my-plugin', fetchFn);
    expect(text).toContain('# Changelog');
    expect(fetchFn.mock.calls[0][0]).toBe('/api/admin/plugins/my-plugin/changelog');
  });
  ...
  it('getChangelog encodes the plugin name', async () => {
    ...
    expect(fetchFn.mock.calls[0][0]).toBe('/api/admin/plugins/my%20plugin/changelog');
  });
});
