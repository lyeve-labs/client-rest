...
import { getSummary, getEndpoints, getTenants, getMethods, getAgents, getTrend, getAnomalies } from '@lyeve/cms-client-rest';
import type { MetricsQuery } from '@lyeve/cms-client-rest';
...
describe('REST API analytics', () => {
  it('getSummary(query, client) GETs /metrics/summary with query string', async () => {
    const { client, fetchFn } = mkClient({ total_requests: 1000, total_2xx: 950, total_4xx: 40, total_5xx: 10, error_rate: 0.01, avg_latency_p50_ms: 50, avg_latency_p95_ms: 200, avg_latency_p99_ms: 500, max_latency_p99_ms: 2000, avg_request_size_bytes: 1024, max_request_size_bytes: 65536, unique_endpoints: 10, unique_tenants: 3, unique_methods: 5, unique_user_agents: 2, from: '2025-01-01T00:00:00Z', to: '2025-01-02T00:00:00Z' });
    const query: MetricsQuery = { tenant_id: 't1', from: '2025-01-01T00:00:00Z', to: '2025-01-02T00:00:00Z' };
  ...
  it('getSummary with no query omits query string', async () => {
    const { client, fetchFn } = mkClient({ total_requests: 0, total_2xx: 0, total_4xx: 0, total_5xx: 0, error_rate: 0, avg_latency_p50_ms: 0, avg_latency_p95_ms: 0, avg_latency_p99_ms: 0, max_latency_p99_ms: 0, avg_request_size_bytes: 0, max_request_size_bytes: 0, unique_endpoints: 0, unique_tenants: 0, unique_methods: 0, unique_user_agents: 0, from: '', to: '' });
  ...
  it('getSummary drops undefined/null/empty values from query', async () => {
    const { client, fetchFn } = mkClient({ total_requests: 0, total_2xx: 0, total_4xx: 0, total_5xx: 0, error_rate: 0, avg_latency_p50_ms: 0, avg_latency_p95_ms: 0, avg_latency_p99_ms: 0, max_latency_p99_ms: 0, avg_request_size_bytes: 0, max_request_size_bytes: 0, unique_endpoints: 0, unique_tenants: 0, unique_methods: 0, unique_user_agents: 0, from: '', to: '' });
    const query: MetricsQuery = { tenant_id: 't1', endpoint: undefined, method: '' };
  ...
  it('getEndpoints GETs /metrics/endpoints', async () => {
    const { client, fetchFn } = mkClient({ items: [{ key: 'GET /api/admin/users', request_count: 100, error_rate: 0.01, avg_latency_p95_ms: 150, max_latency_p99_ms: 500, request_size_avg_bytes: 500 }], total: 1 });
  ...
  it('getTrend GETs /metrics/trend', async () => {
    const { client, fetchFn } = mkClient({ points: [{ hour: '2025-01-01T10:00:00Z', request_count: 200, error_rate: 0.02, avg_latency_p95_ms: 180, max_latency_p99_ms: 600 }] });
  ...
  it('getAnomalies GETs /metrics/anomalies', async () => {
    const { client, fetchFn } = mkClient({ anomalies: [{ hour: '2025-01-01T12:00:00Z', metric_name: 'error_rate', actual_value: 0.15, expected_avg: 0.02, z_score: 4.2, severity: 'high' }], window_hours: 72, z_threshold: 3.0 });
