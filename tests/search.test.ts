...
import { search, listSynonyms, getRanking } from "../src/index.js";
...
describe("REST search", () => {
  it("search GETs /api/admin/search?q={query}", async () => {
    const { client, fetchFn } = mkClient({
      results: [{ entry_id: "e1", schema: "articles", title: "Hello" }],
      total: 1,
      limit: 25,
      offset: 0,
      query: "hello",
  ...
  it("getRanking(client, schema) GETs /api/admin/search/ranking?schema=X", async () => {
    ...
    expect(fetchFn.mock.calls[0][0]).toBe(
      "/api/admin/search/ranking?schema=articles",
    );
    expect(fetchFn.mock.calls[0][1].method).toBe("GET");
  });
  ...
  it("getRanking encodes the schema name", async () => {
    ...
    expect(fetchFn.mock.calls[0][0]).toBe(
      "/api/admin/search/ranking?schema=my%20schema",
    );
  });
});
