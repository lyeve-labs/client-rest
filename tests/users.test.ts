...
import { getUsers, createUser, updateUserRoles, deleteUser } from '@lyeve/cms-client-rest';
...
describe('REST users', () => {
  ...
  it('createUser POSTs email, password, roles', async () => {
    const { client, fetchFn } = mkClient({ id: 'u2', email: 'new@b.co', roles: ['editor'] });
    const user = await createUser('new@b.co', 'p4ss', ['editor'], client);
    expect(user.email).toBe('new@b.co');
    expect(fetchFn.mock.calls[0][0]).toBe('/api/admin/users');
    expect(fetchFn.mock.calls[0][1].method).toBe('POST');
    expect(JSON.parse(fetchFn.mock.calls[0][1].body)).toEqual({ email: 'new@b.co', password: 'p4ss', roles: ['editor'] });
  });

  it('updateUserRoles PUTs roles to /api/admin/users/{id}/roles', async () => {
    const { client, fetchFn } = mkClient({ id: 'u1', roles: ['admin', 'editor'] });
    ...
    expect(JSON.parse(fetchFn.mock.calls[0][1].body)).toEqual({ roles: ['admin', 'editor'] });
  });
