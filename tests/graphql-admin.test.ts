...
import { listPersistedQueries, createPersistedQuery, deletePersistedQuery, updatePersistedQuery } from '@lyeve/cms-client-rest';
...
describe('REST GraphQL admin', () => {
  it('listPersistedQueries GETs /api/admin/graphql/persisted-queries', async () => {
    const { client, fetchFn } = mkClient([{ id: 'pq1', name: 'GetContent', query: 'query { content { id } }', created_at: '', updated_at: '' }]);
    const queries = await listPersistedQueries(client);
    expect(queries).toHaveLength(1);
    expect(queries[0].name).toBe('GetContent');
    expect(fetchFn.mock.calls[0][0]).toBe('/api/admin/graphql/persisted-queries');
    expect(fetchFn.mock.calls[0][1].method).toBe('GET');
  });

  it('createPersistedQuery POSTs input to /api/admin/graphql/persisted-queries', async () => {
    const { client, fetchFn } = mkClient({ id: 'pq2', name: 'UpdateContent', query: 'mutation { updateContent }', created_at: '', updated_at: '' });
    const input = { name: 'UpdateContent', query: 'mutation { updateContent }', variables: { id: '123' } };
  ...
  it('deletePersistedQuery DELETEs /api/admin/graphql/persisted-queries/{id}', async () => {
    ...
    expect(fetchFn.mock.calls[0][0]).toBe('/api/admin/graphql/persisted-queries/pq1');
  ...
  it('deletePersistedQuery encodes the query id', async () => {
    ...
    expect(fetchFn.mock.calls[0][0]).toBe('/api/admin/graphql/persisted-queries/pq%2F1');
  });

  it('updatePersistedQuery PATCHes input to /api/admin/graphql/persisted-queries/{id}', async () => {
    const { client, fetchFn } = mkClient({ id: 'pq1', name: 'Renamed', query: 'query { content { id title } }', created_at: '', updated_at: '' });
  ...
  it('updatePersistedQuery sends partial update', async () => {
    ...
    expect(JSON.parse(fetchFn.mock.calls[0][1].body)).toEqual({ name: 'JustName' });
  });
});
