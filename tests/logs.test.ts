...
describe("REST logs", () => {
  it("searchLogs(client, params) GETs /api/admin/logs/search with query params", async () => {
    const { client, fetchFn } = mkClient({
      results: [
        { timestamp: "2025-01-01T00:00:00Z", level: "ERROR", message: "boom" },
      ],
      total: 1,
      limit: 100,
      offset: 0,
    });
    const result = await searchLogs(client, {
      query: "boom",
      level: "ERROR",
      limit: 50,
    });
  ...
  it("getLoggingConfig GETs /api/admin/logging/config", async () => {
    const { client, fetchFn } = mkClient({
      sinks: [{ driver: "stdout" }],
      levels: { default_level: 0, tenants: {}, plugins: {} },
  ...
  it("getLogVolume(client, window) GETs /api/admin/logging/volume?window=...", async () => {
    ...
    expect(fetchFn.mock.calls[0][0]).toBe(
      "/api/admin/logging/volume?window=1h",
    );
    expect(fetchFn.mock.calls[0][1].method).toBe("GET");
  });
  ...
  it("getLogVolume encodes the window parameter", async () => {
    ...
    expect(fetchFn.mock.calls[0][0]).toBe(
      "/api/admin/logging/volume?window=24h",
    );
  });
});
