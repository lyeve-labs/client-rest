...
export function listPersistedQueries(client: HttpClient): Promise<PersistedQuery[]> {
...
export function createPersistedQuery(
	...
	return client.post<PersistedQuery>('/api/admin/graphql/persisted-queries', input);
}

/** DELETE /api/admin/graphql/persisted-queries/{id} - delete a persisted query. */
export function deletePersistedQuery(id: string, client: HttpClient): Promise<void> {
