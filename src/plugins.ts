import { ApiError, type HttpClient } from '@lyeve/cms-client';
...
export function getPluginStatus(client: HttpClient): Promise<PluginStatusReport> {
	return client.get<PluginStatusReport>('/api/admin/plugins/status');
}

/** Fetch the JSON Schema describing a plugin's configuration. */
export function getPluginSchema(name: string, client: HttpClient): Promise<JsonSchema> {
	return client.get<JsonSchema>(`/api/admin/plugins/${encodeURIComponent(name)}/schema`);
}
...
export async function getChangelog(
	...
	const res = await fetchFn(`/api/admin/plugins/${encodeURIComponent(name)}/changelog`);
	if (res.status === 404) return '';
	if (!res.ok) {
		throw new ApiError(res.status, await res.text().catch(() => 'Failed to load changelog'));
	}
	return res.text();
}
