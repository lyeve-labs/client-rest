...
export type ProviderType = 'openai' | 'anthropic' | 'google' | 'ollama' | 'groq' | 'deepseek';
...
export type FallbackCondition = 'error_rate' | 'latency_exceeded' | 'rate_limited' | 'always';
...
export function updateProvider(
	...
	return client.put<Provider>(`/api/admin/providers/${encodeURIComponent(id)}`, input);
}
...
export async function getMetrics(client: HttpClient): Promise<MetricsListResult> {
	const res = await client.get<{ data: MetricsListResult }>('/api/admin/providers/metrics');
...
export async function listFallbackRules(client: HttpClient): Promise<FallbackRule[]> {
	const res = await client.get<Paginated<FallbackRule>>('/api/admin/providers/fallback-rules');
	return res.data ?? [];
}
