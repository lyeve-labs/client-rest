...
export function search(q: string, client: HttpClient): Promise<SearchResponse> {
	return client.get<SearchResponse>(`/api/admin/search?q=${encodeURIComponent(q)}`);
}
...
export function getRanking(client: HttpClient, schema = '*'): Promise<RankingConfig> {
	return client.get<RankingConfig>(`/api/admin/search/ranking?schema=${encodeURIComponent(schema)}`);
}
